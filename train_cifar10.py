...
# fix for running on Windows by secutron@naver.com
...
def main():
    ...
    def sanitize_args(obj):
        if isinstance(obj, str):
            ...
            return {k: sanitize_args(v) for k, v in obj.items()}
        elif isinstance(obj, (list, tuple)):
            return type(obj)(sanitize_args(x) for x in obj)
        else:
            return obj
    ...
    if usewandb:
        ...
        clean_args = sanitize_args(vars(args))    
        
        watermark = "{}_lr{}_{}".format(clean_args['net'], clean_args['lr'], clean_args['dataset'])
        wandb.init(project="cifar-challenge",
                name=watermark)
        wandb.config.update(clean_args)
